...
resultsDataPerAlgorithm <- split(resultsData$AUCPrecisionRecall, resultsData$Algorithm, drop = TRUE)
...
holmTest <- pairwise.t.test(AUCPrecisionRecall, Algorithm, p.adj = "holm")
#kruskal.test(Ozone ~ Month, data = airquality)

png(file = "aucpr_algorithm_comparison.png")
first=TRUE
i <- 1
...
for(algorithm in labels){
  ...
  if(first){
    first=FALSE
    plot(algAucRocDensity, col=colors[i], main="AUC Precision/Recall by Algorithm", xlim=c(0.87,1), ylim=c(0,120), xlab="AUC ROC")
  } else {
...
write.table(format(pValueDataFrame, digits=6), "aucPrecisionRecallAlgComparison.csv", row.names=FALSE, col.names=TRUE, quote=FALSE, sep=",", append = FALSE)