...
p + scale_y_continuous(limits = quantile(nscData$NSC, c(0.01, 0.9)))
...
p + theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.title = element_text(hjust = 0.5)) + geom_text(data = means, aes(label = round(RSC, digits = 3), y = RSC, x = Treatment))
# + scale_y_continuous(limits = quantile(rscData$RSC, c(0.1, 0.9)))
